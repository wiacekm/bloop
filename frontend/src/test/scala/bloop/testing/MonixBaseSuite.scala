...
import bloop.logging.{BspServerLogger, RecordingLogger}
...
import utest.framework.{Formatter, TestCallTree, Tree}
...
abstract class MonixBaseSuite extends MonixTaskTest with BloopHelpers {
  override implicit val scheduler: Scheduler =
    Scheduler.global
  override val timeout: FiniteDuration = 300.second
  ...
  case class FlatTest(name: String, thunk: () => Task[Any])
  private val myTests = IndexedSeq.newBuilder[FlatTest]

  @nowarn("msg=parameter value fun in method ignore is never used")
  def ignore(name: String, label: String = "IGNORED")(fun: => Task[Any]): Unit = {
    test(utest.ufansi.Color.LightRed(s"$label - $name").toString())(Task.unit)
  }

  def testOnlyOnJava8(name: String)(fun: => Task[Any]): Unit = {
    if (TestUtil.isJdk8) test(name)(fun)
    else ignore(name, label = s"IGNORED ON JAVA v${TestUtil.jdkVersion}")(fun)
  }

//  def flakyTest(name: String, attempts: Int)(fun: => Task[Any]): Unit = {
//    assert(attempts >= 0)
//    def retry(fun: => Task[Any], attempts: Int): Task[Any] = {
//      try {
//        fun; ()
//      } catch {
//        case NonFatal(t) =>
...
//  }
  ...
  def test(name: String)(fun: => Task[Any]): Unit = {
